...
class Solution:
    def simplifyPath(self, path: str) -> str:
        # create an empty stack to store directory
        stack = []
        ...
        # For element in path
        for element in path:
            # if stack is not empty and element is '..' then it represent the home directory so pop the last path
            if stack and element == '..':
                stack.pop()
            # if element not ['.', '', '..'] -> then it means name of the path so add to the stack
            elif element not in ['.', '', '..']:
                stack.append(element)

        # return stack element with '/' (path directory always starts with slash) and join other element with '/'
        return '/' + '/'.join(stack)


solution = Solution()
assert '/home/foo' == solution.simplifyPath(path="/home//foo/")
assert '/' == solution.simplifyPath(path="/../")
assert '/home' == solution.simplifyPath(path="/home/")
