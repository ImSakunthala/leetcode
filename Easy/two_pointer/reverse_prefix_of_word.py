    ...
    def reversePrefix(self, word: str, ch: str) -> str:
        if ch not in word:
            ...
            for index, char in enumerate(word):
                if char == ch:
                    result.insert(1, word[index + 1:])
                    result.insert(0, word[:index + 1][::-1])
                    ch = ''
            return ''.join(result)


solution = Solution()
assert 'dcbaefd' == solution.reversePrefix(word="abcdefd", ch="d")
assert 'zxyxxe' == solution.reversePrefix(word="xyxzxe", ch="z")
assert 'abcd' == solution.reversePrefix(word="abcd", ch="z")
