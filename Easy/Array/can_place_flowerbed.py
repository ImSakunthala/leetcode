...
class Solution:
    def canPlaceFlowers(self, flowerbed: List[int], n: int) -> bool:
        ...
        for index, i in enumerate(flowerbed):
            if i == 0 and (index == 0 or flowerbed[index-1] == 0) and (index == len(flowerbed)-1 or flowerbed[index+1]== 0):
                flowerbed[index] = 1
                count += 1
                n -= 1
                if n == 0:
...
solution = Solution()
assert solution.canPlaceFlowers(flowerbed=[1], n=1) == False
assert solution.canPlaceFlowers(flowerbed=[0], n=1) == True
assert solution.canPlaceFlowers(flowerbed=[1, 0, 0, 0, 1, 0, 0], n=2) == True
