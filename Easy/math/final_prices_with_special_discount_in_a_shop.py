...
class Solution:
    def finalPrices(self, prices: List[int]) -> List[int]:
        final_price = [0] * len(prices)

        for i in range(len(prices)):
            for price in prices[i + 1:]:
                if prices[i] >= price:
                    final_price[i] = prices[i] - price
                    break
            else:
...
solution = Solution()
assert [4, 2, 4, 2, 3] == solution.finalPrices(prices=[8, 4, 6, 2, 3])
assert [1, 2, 3, 4, 5] == solution.finalPrices(prices=[1, 2, 3, 4, 5])
assert [9, 0, 1,  6] == solution.finalPrices(prices=[10, 1, 1, 6])
