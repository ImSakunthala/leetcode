...
class Solution:
    def decompressRLElist(self, nums: List[int]) -> List[int]:
        ...
        while position < len(nums)//2:
            result.extend(nums[2*position]*[nums[2*position+1]])
            position += 1
        return result


solution = Solution()
assert [2, 4, 4, 4] == solution.decompressRLElist(nums = [1,2,3,4])
assert [1, 3, 3] == solution.decompressRLElist(nums = [1,1,2,3])
