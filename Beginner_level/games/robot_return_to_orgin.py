...
Input: moves = "UD" Output: true Explanation: The robot moves up once, and then down once. All moves have the same
magnitude, so it ended up at the origin where it started. Therefore, we return true.

Example 2:

Input: moves = "LL" Output: false Explanation: The robot moves left twice. It ends up two "moves" to the left of the
...
class Solution:
    def judgeCircle(self, moves: str) -> bool:
        ...
        for direction in moves:
            move_map[direction] = move_map.get(direction, 0)+1

        if move_map.get('U', 0) == move_map.get('D', 0) and move_map.get('R', 0) == move_map.get('L', 0):
            return True
        return False


solution = Solution()
print(solution.judgeCircle("UDDUURLRLLRRUDUDLLRLURLRLRLUUDLULRULRLDDDUDDDDLRRDDRDRLRLURRLLRUDU"
                           "RULULRDRDLURLUDRRLRLDDLUUULUDUUUUL"))
